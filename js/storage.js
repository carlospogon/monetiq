...
export class Storage {
    ...
    clearAll() {
        console.log('Storage: Clearing all tickets from localStorage');
        this.cache = [];
        this.save();
    }
    ...
    getMerchants() {
        const merchants = new Set(this.cache.map(t => t.merchant));
        return Array.from(merchants).sort();
    }

    getStats(merchantFilter = null) {
        const filteredTickets = merchantFilter
            ? this.cache.filter(t => t.merchant === merchantFilter)
            : this.cache;

        const totalExpense = filteredTickets.reduce((sum, t) => sum + t.total, 0);
        const categories = {};
        ...
        // Top expensive items
        const allItems = filteredTickets.flatMap(t => t.items);
        allItems.sort((a, b) => b.price - a.price);

        const avgTicket = filteredTickets.length > 0 ? totalExpense / filteredTickets.length : 0;
