...
class App {
    ...
    async init() {
        ...
        // Attach listeners FIRST to ensure they work even if rendering fails
        document.getElementById('save-ticket-btn').addEventListener('click', () => this.saveCurrentTicket());
        ...
        // Analytics Upload Button
        const analyticsUploadBtn = document.getElementById('analytics-upload-btn');
        if (analyticsUploadBtn) {
            analyticsUploadBtn.addEventListener('click', () => {
        ...
        if (clearHistoryBtn) {
            console.log('Attaching Clear History Listener');
    ...
    async handleBatch(files) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                if (file.type.includes('image')) {
                    // Show preview of current image being processed
                    previewEl.src = URL.createObjectURL(file);
                    text = await this.ocr.processImage(file);
                    ...
                    text = await this.ocr.processPDF(file);
                }
                ...
                aggregatedItems.push(...rawData.items);
                batchTotal += rawData.total;
                if (rawData.merchant !== 'Desconocido') merchants.add(rawData.merchant);
                if (rawData.date) dates.add(rawData.date);
        ...
        statusEl.textContent = `Lote completado. ${totalProcessed} tickets procesados.`;

        // Aggregate/Consolidate items
        const consolidatedItems = this.consolidateItems(aggregatedItems);

        // construct "Batch Ticket"
        this.currentTicket = {
            merchant: merchants.size > 0 ? Array.from(merchants).join(' + ') : 'Lote Varios',
            date: dates.size > 0 ? Array.from(dates).sort()[0] : new Date().toISOString().split('T')[0],
            total: batchTotal,
            items: consolidatedItems,
            isBatch: true // Flag to know it's a batch
        };
    ...
    loadDashboard() {
        ...
        const timeStats = new AnalyticsEngine(this.storage.getAll()).getTimeAnalysis();
        if (timeStats) {
        ...
        const predictor = new ProductPredictor(tickets);
